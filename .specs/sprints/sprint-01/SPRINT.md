...
Entregar a base de autenticação e o pipeline de CI/CD do <PRODUCT_NAME> para destravar todas as features futuras de <DOMAIN>.
...
3. **Deploy automático em staging** — push em `main` dispara deploy para ambiente `staging.<PRODUCT_NAME>.app` via GitHub Actions.
...
| -------------------------- | ------ | ----------- |
| `01-example.task.md`       | doing  | @<TEAM>     |
| `02-ci-pipeline.task.md`   | todo   | @<TEAM>     |
| `03-staging-deploy.task.md`| todo   | @<TEAM>     |
| `04-docs-baseline.task.md` | todo   | @<TEAM>     |
...
- **Time pequeno e curva de aprendizado da stack `<STACK>`.** Mitigação: pareamento nas 2 primeiras tasks.

## Dependências

- ADR-001 sobre escolha de stack precisa estar `accepted` antes da task de pipeline.
- Domínio `staging.<PRODUCT_NAME>.app` precisa estar provisionado.
- Acesso de `<TEAM>` ao repositório com permissão de deploy.
